...
var Attribute$1;
(function(Attribute3) {
  Attribute3["MAYBE_CLICK_EVENT_TYPE"] = "maybe_click";
  Attribute3["SKIP_GLOBAL_DISPATCH"] = "a11ysgd";
  Attribute3["SKIP_A11Y_CHECK"] = "a11ysc";
...
var KeyCode;
(function(KeyCode2) {
  KeyCode2[KeyCode2["MAC_ENTER"] = 3] = "MAC_ENTER";
...
function createCustomEvent(type, data, triggeringEvent) {
  ...
  const detail = __spreadValues(__spreadValues({}, unrenamedDetail), renamedDetail);
  try {
...
function isKeyboardEvent(eventType) {
  return eventType === EventType.KEYPRESS || eventType === EventType.KEYDOWN || eventType === EventType.KEYUP;
}
function isMouseEvent(eventType) {
  return eventType === EventType.CLICK || eventType === EventType.DBLCLICK || eventType === EventType.MOUSEDOWN || eventType === EventType.MOUSEOVER || eventType === EventType.MOUSEOUT || eventType === EventType.MOUSEMOVE;
}
function isUiEvent(eventType) {
  return eventType === EventType.FOCUS || eventType === EventType.BLUR || eventType === EventType.FOCUSIN || eventType === EventType.FOCUSOUT || eventType === EventType.SCROLL;
}
...
function createUiEvent(original, opt_eventType) {
  ...
  if (document.createEvent) {
    ...
    event.initUIEvent(opt_eventType || originalUiEvent.type, originalUiEvent.bubbles !== void 0 ? originalUiEvent.bubbles : true, originalUiEvent.cancelable || false, originalUiEvent.view || window, original.detail || 0);
  } else {
    event = document.createEventObject();
    event.type = opt_eventType || original.type;
    event.bubbles = original.bubbles !== void 0 ? original.bubbles : true;
...
(function(Restriction2) {
  Restriction2[Restriction2["I_AM_THE_JSACTION_FRAMEWORK"] = 1] = "I_AM_THE_JSACTION_FRAMEWORK";
...
var BaseDispatcher = class {
   ...
   */
  constructor(dispatchDelegate, { eventReplayer = void 0 } = {}) {
  ...
  dispatch(eventInfo, isGlobalDispatch) {
    ...
    if (eventInfoWrapper.getIsReplay()) {
      ...
      const resolved2 = resolveA11yEvent(eventInfoWrapper, isGlobalDispatch);
      if (!resolved2) {
    ...
    if (!resolved) {
      eventInfoWrapper.setAction(void 0);
   ...
   */
  scheduleEventReplay() {
    if (this.eventReplayScheduled || !this.eventReplayer || this.queuedEventInfoWrappers.length === 0) {
      return;
    }
...
function isA11yClickEvent(eventInfoWrapper, isGlobalDispatch) {
  return (isGlobalDispatch || eventInfoWrapper.getAction() !== void 0) && isActionKeyEvent(eventInfoWrapper.getEvent());
}
var Char = {
  /**
...
var Dispatcher = class {
   ...
   *     given event info.
   */
  constructor(getHandler, { stopPropagation: stopPropagation2 = false, eventReplayer = void 0 } = {}) {
    ...
    this.stopPropagation = stopPropagation2;
  }
  /**
   * Receives an event or the event queue from the EventContract. The event
   * queue is copied and it attempts to replay.
  ...
  dispatchToHandler(eventInfoWrapper, isGlobalDispatch) {
    if (isGlobalDispatch) {
      ...
      let shouldPreventDefault2 = false;
      if (eventTypeHandlers) {
        for (const handler2 of eventTypeHandlers) {
          if (handler2(ev) === false) {
            shouldPreventDefault2 = true;
          }
        }
      }
      if (shouldPreventDefault2) {
        preventDefault(ev);
      }
      return;
    }
  ...
  registerGlobalHandler(eventType, handler) {
    if (!this.globalHandlers.has(eventType)) {
      this.globalHandlers.set(eventType, /* @__PURE__ */ new Set([handler]));
    } else {
...
function stopPropagation(eventInfoWrapper) {
  if (isGecko && (eventInfoWrapper.getTargetElement().tagName === "INPUT" || eventInfoWrapper.getTargetElement().tagName === "TEXTAREA") && eventInfoWrapper.getEventType() === EventType.FOCUS) {
    return;
  }
...
var Attribute;
(function(Attribute3) {
  Attribute3["JSACTION"] = "jsaction";
  Attribute3["JSNAMESPACE"] = "jsnamespace";
  Attribute3["OI"] = "oi";
  Attribute3["VED"] = "ved";
  Attribute3["VET"] = "vet";
  Attribute3["JSINSTANCE"] = "jsinstance";
  Attribute3["JSTRACK"] = "jstrack";
})(Attribute || (Attribute = {}));
var Property;
(function(Property2) {
  Property2["JSACTION"] = "__jsaction";
  Property2["JSNAMESPACE"] = "__jsnamespace";
  Property2["OI"] = "__oi";
  Property2["OWNER"] = "__owner";
})(Property || (Property = {}));
var parseCache = {};
...
var JSNAMESPACE_SUPPORT = true;
var A11Y_SUPPORT_IN_DISPATCHER = false;
var A11Y_CLICK_SUPPORT_FLAG_ENABLED = false;
var A11Y_CLICK_SUPPORT = A11Y_CLICK_SUPPORT_FLAG_ENABLED || A11Y_SUPPORT_IN_DISPATCHER;
var MOUSE_SPECIAL_SUPPORT = false;
var STOP_PROPAGATION = true;
var CUSTOM_EVENT_SUPPORT = false;
...
var _EventContract = class _EventContract {
  ...
  handleEventInfo(eventInfo, allowRehandling = true) {
    ...
    if (_EventContract.CUSTOM_EVENT_SUPPORT && getEventType(eventInfo) === EventType.CUSTOM) {
    ...
    if (this.dispatcher && !getEvent(eventInfo)[Attribute$1.SKIP_GLOBAL_DISPATCH]) {
    ...
    if (this.dispatcher) {
      if (action && shouldPreventDefaultBeforeDispatching(getActionElement(action), eventInfo)) {
        preventDefault(getEvent(eventInfo));
      }
      const unresolvedEventInfo = this.dispatcher(eventInfo);
      if (unresolvedEventInfo && allowRehandling) {
        this.handleEventInfo(
          unresolvedEventInfo,
          /* allowRehandling= */
          false
        );
  ...
  populateAction(eventInfo) {
    ...
    } else if (_EventContract.A11Y_SUPPORT_IN_DISPATCHER && getEventType(eventInfo) === EventType.KEYDOWN && !getEvent(eventInfo)[Attribute$1.SKIP_A11Y_CHECK]) {
    ...
    if (_EventContract.MOUSE_SPECIAL_SUPPORT && (getEventType(eventInfo) === EventType.MOUSEENTER || getEventType(eventInfo) === EventType.MOUSELEAVE || getEventType(eventInfo) === EventType.POINTERENTER || getEventType(eventInfo) === EventType.POINTERLEAVE)) {
  ...
  populateActionOnElement(actionElement, eventInfo) {
    ...
    if (_EventContract.A11Y_SUPPORT_IN_DISPATCHER) {
      if (getEventType(eventInfo) === Attribute$1.MAYBE_CLICK_EVENT_TYPE && actionMap[EventType.CLICK] !== void 0) {
...
_EventContract.CUSTOM_EVENT_SUPPORT = CUSTOM_EVENT_SUPPORT;
_EventContract.STOP_PROPAGATION = STOP_PROPAGATION;
_EventContract.A11Y_SUPPORT_IN_DISPATCHER = A11Y_SUPPORT_IN_DISPATCHER;
_EventContract.A11Y_CLICK_SUPPORT = A11Y_CLICK_SUPPORT;
_EventContract.MOUSE_SPECIAL_SUPPORT = MOUSE_SPECIAL_SUPPORT;
_EventContract.JSNAMESPACE_SUPPORT = JSNAMESPACE_SUPPORT;
var EventContract = _EventContract;
function checkDispatcherForA11yClick(eventInfo) {
  return EventContract.A11Y_SUPPORT_IN_DISPATCHER && getEventType(eventInfo) === Attribute$1.MAYBE_CLICK_EVENT_TYPE;
}
function shouldPreventDefaultBeforeDispatching(actionElement, eventInfo) {
  return actionElement.tagName === "A" && (getEventType(eventInfo) === EventType.CLICK || getEventType(eventInfo) === EventType.CLICKMOD);
}
...
function scheduleCallbackWithRafRace(callback, useNativeTimers = true) {
  ...
  if (typeof Zone !== "undefined" && useNativeTimers) {
    if (hasRequestAnimationFrame) {
      nativeRequestAnimationFrame = _global[Zone.__symbol__("requestAnimationFrame")] ?? nativeRequestAnimationFrame;
    }
...
var ComponentFactory = class extends ComponentFactory$1 {
  ...
  create(injector, projectableNodes, rootSelectorOrNode, environmentInjector) {
    ...
    try {
      ...
      const hostRNode = rootSelectorOrNode ? locateHostElement(hostRenderer, rootSelectorOrNode, this.componentDef.encapsulation, rootViewInjector) : createElementNode(hostRenderer, elementName, getNamespace2(elementName));
      let rootFlags = 512;
...
@angular/core/fesm2022/core.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-2IL6UC6U.js.map
